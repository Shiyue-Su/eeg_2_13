...
class EEG_Dataset(Dataset):
    def __init__(self, cfg, train_subs=None, val_subs=None, sliced=True, mods=None, return_meta=False):
        ...
        self.onesubLen = len(self.onesub_labels)
        self.n_samples_onesub = np.load(os.path.join(self.sliced_data_dir, 'metadata', 'n_samples_onesub.npy')).astype(int)
        self.n_samples_onesub_cum = np.concatenate((np.array([0]), np.cumsum(self.n_samples_onesub)))
    ...
    def _map_to_abs_index(self, idx):
        if self.mods == 'train':
            if self.train_subs is not None:
                idx = self.train_subs[idx//self.onesubLen]*self.onesubLen + idx%self.onesubLen
        elif self.mods == 'val':
    ...
    def _decode_index(self, abs_idx):
        subject_id = int(abs_idx // self.onesubLen)
        idx_in_subject = int(abs_idx % self.onesubLen)
        trial_id = int(np.searchsorted(self.n_samples_onesub_cum, idx_in_subject, side='right') - 1)
        trial_id = max(0, min(trial_id, len(self.n_samples_onesub) - 1))
        time_idx = int(idx_in_subject - self.n_samples_onesub_cum[trial_id])
        return {
            'abs_idx': torch.tensor(abs_idx, dtype=torch.long),
    ...
    def __getitem__(self, idx):
        idx = self._map_to_abs_index(idx)
        one_seq = np.load(os.path.join(self.sliced_data_dir, 'data', f'data_sample_{idx}.npy'))
        one_seq = torch.FloatTensor(one_seq.reshape(1,one_seq.shape[-2],one_seq.shape[-1]))    # 32*(250)->1*32*250  2d conv  c*h*w
        one_label = self.labels[idx]
        if not self.return_meta:
            return one_seq, one_label
        meta = self._decode_index(idx)
        return one_seq, one_label, meta
