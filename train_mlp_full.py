...
def train_mlp(cfg: DictConfig) -> None:
    ...
    for fold in range(n_folds):
        ...
        save_path_primary = os.path.join(
            save_dir,
        ...
        save_path_alt = os.path.join(
            save_dir,
            f"{cfg.log.run_name}_{f'f{fold}' if cfg.val.extractor.normTrain else 'all'}_fea_"
            + (f"epoch={(cfg.val.extractor.ckpt_epoch-1):02d}.ckpt" if cfg.val.extractor.use_pretrain else "")
            + f"{cfg.val.extractor.fea_mode}.npy"
        )
        save_path = save_path_primary if os.path.exists(save_path_primary) else save_path_alt
        if not os.path.exists(save_path):
            raise FileNotFoundError(
                f"Feature file not found. Tried:\n- {save_path_primary}\n- {save_path_alt}\n"
                "Please run ext_fea.py first with matching config."
            )
