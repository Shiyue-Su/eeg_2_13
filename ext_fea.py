...
def ext_fea(cfg: DictConfig) -> None:
    ...
    if cfg.val.extractor.use_pretrain:
        ...
        if not os.path.exists(cp_path):
            raise FileNotFoundError(
                ...
                'Please run train_multi.py first or set val.extractor.use_pretrain=False.'
            )
    ...
    for fold in tqdm(range(n_folds), desc='Extracting feature......'):
        ...
        if np.isnan(fea).any():
            ...
            print('no nan')
        ckpt_tag = f"epoch={(cfg.val.extractor.ckpt_epoch-1):02d}.ckpt" if cfg.val.extractor.use_pretrain else ""
        mode_tag = cfg.val.extractor.fea_mode
        if not cfg.val.extractor.normTrain:
            save_name = f"{cfg.log.run_name}_all_fea_{ckpt_tag}{mode_tag}.npy"
        else:
            save_name = f"{cfg.log.run_name}_f{fold}_fea_{ckpt_tag}{mode_tag}.npy"
